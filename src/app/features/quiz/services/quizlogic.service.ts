...
import { TranslationService } from '../../../services/translation/translation-service.service'; // Import der TranslationService
import { map } from 'rxjs/operators'; // Import von map aus rxjs
import { Fragen } from './../../../../app/features/quiz/models/fragen'; // Import der Fragen-Schnittstelle
...
export class QuizlogicService {
  ...
  toggleQuiz(selectedCase: number) {
    // Methode zum Starten des Quizzes
    let selectedArray: Fragen[]; // Deklaration einer lokalen Variablen für das ausgewählte Array von Fragen

    switch (
      selectedCase // Verwendung eines switch-Statements, um das entsprechende Array von Fragen basierend auf dem ausgewählten Fall auszuwählen
    ) {
      case 15: // Fall 15:Quiz Snipped Fragenarray
        this.translationService
          .getTranslation('qSnipped')
          .pipe(
            // Verwendung der getTranslation-Methode, um die entsprechende Übersetzung zu laden mit den key "qSnipped"
            map((fragen: Fragen[]) => {
              // Verwendung der map-Methode, um das Observable zu transformieren
          ...
          .subscribe(
            // Abonniere das Observable und lade die Übersetzungen
            () => {}, // Leerer Erfolgs-Callback, kann optional für weitere Aktionen verwendet werden
            (error) =>
              console.error('Fehler beim Laden der Übersetzungen:', error) // Konsolenausgabe des Fehlers
          );
        break;
      case 0:
  ...
  initializeQuiz() {
    // Methode zum Initialisieren des Quizzes
  ...
  ladeFrage() {
    ...
    try {
      if (this.Fragen.length > this.aktuelleFrageIndex) {
        // Überprüft ob alle Fragen durchgelaufen sind
      ...
      if (this.aktuelleFrageIndex === this.Fragen.length) {
        // Überprüft, ob alle Fragen einmal angezeigt wurden
        if (this.unbeantworteteFragen.length === this.skipFragenIndex) {
          // Überprüft ob es skipped Fragen gibt
  ...
  fragenNummer(): number | string {
    // Methode zum Abrufen der aktuellen Fragennummer
    try {
      ...
      if (aktuelleFragennummer <= gesamtanzahlFragen) {
        // Überprüfe, ob die aktuelle Fragennummer innerhalb des gültigen Bereichs liegt
    ...
    try {
      ...
      if (this.Fragen.length > this.aktuelleFrageIndex) {
        ...
        if (
          ...
          // Überprüft die Antwort mit der Hinterlegten im Array
        ...
        // Überprüfen der Antwort für übersprungene Fragen
        if (
  ...
  nextFrage() {
    // Methode zum Überspringen der aktuellen Frage
    try {
      if (!this.Fragen[this.aktuelleFrageIndex].uebersprungen) {
        // Überprüfen, ob die aktuelle Frage bereits übersprungen wurde
      ...
      if (this.aktuelleFrageIndex < this.Fragen.length) {
        // Überprüfen, ob es noch weitere Fragen gibt, und ggf. zur nächsten Frage wechseln
  ...
  skipFragen() {
    // Methode zum Anzeigen der übersprungenen Fragen
    try {
      if (this.skipFragenIndex === this.unbeantworteteFragen.length) {
        // Prüft, ob alle übersprungenen Fragen angezeigt wurden
      ...
      if (this.unbeantworteteFragen.length > this.skipFragenIndex) {
        // Prüft, ob noch weitere übersprungenen Fragen verfügbar sind
        this.skipFrage = this.unbeantworteteFragen[this.skipFragenIndex].frage; // Lade die aktuelle Übersprungene Frage
        this.skipAntwort =
          this.unbeantworteteFragen[this.skipFragenIndex].antwort; // Lade die aktuelle Übersprungene Antwort
  ...
  neustart() {
    // Methode zum Neustarten des Quiz-
    this.Fragen.forEach((frage: Fragen) => {
      // Schleife durch alle Fragen des Arrays
    ...
    console.log(
      ...
      this.Fragen
    ); // Ausgabe der Fragen und der nicht beantworteten Fragen in der Konsole
  }
}
